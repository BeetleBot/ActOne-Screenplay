...
class ImageWidget extends WidgetType {
  constructor(readonly src: string, readonly alt: string, readonly resolver?: (url: string) => string) {
    super();
  }
...
function buildImageBlocks(state: EditorState, config: ImageBlocksConfig): DecorationSet {
  ...
  tree.iterate({
    enter: (node) => {
      ...
      ranges.push(
        Decoration.widget({
          widget: new ImageWidget(src, alt, config.urlResolver),
          block: true,
...
const imageBlocksField = (config: ImageBlocksConfig) => StateField.define<DecorationSet>({
...
export function imageBlocks(config: ImageBlocksConfig = {}): Extension {
  return [imageBlocksField(config), treeProgressPlugin];
}
