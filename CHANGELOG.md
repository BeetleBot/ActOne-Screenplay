...
- 🔐 **Transparent AES-GCM Encryption for AI API Keys** – Integrated installation-bound AES-256-GCM encryption using standard Web Crypto (`window.crypto.subtle`) for AI API keys in `localStorage`, eliminating plaintext exposure while ensuring full cross-platform compatibility across Windows, Linux (Flatpak, AppImage), and headless/minimal desktop setups without OS keyring daemon dependencies.
- 🐞 **Direct Bug Reporting & Feedback via Sentry** – Updated "Report a Bug" to submit user reports directly via Sentry User Feedback alongside system diagnostics and logs without screenplay content, eliminating reliance on vulnerable hardcoded Discord webhooks.
